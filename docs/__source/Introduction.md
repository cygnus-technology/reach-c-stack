...
The Reach stack assumes the support of some sort of transport layer.  The transport layer is responsible for delivering a complete Reach packet.  In the case of a BLE system, the BLE protocol acts as a transport layer.  BLE 5.x supports packet sizes up to 244 bytes.  The default configuration of the Reach stack is tuned to this transport buffer size.

J1939 over CAN is an alternate transport layer.  J1939 includes a transport protocol which can be used to guarantee the delivery of complete packets.  As the J1939 transport protocol supports larger packets, a Reach system that relied only on J1939 might configure Reach to use larger packets.  Reach supports this kind of tuning via the options file provided with reach.proto.  This type of tuning is appropriate only in special cases.
...
Reach conceptually uses six buffers to exchange prompts and responses.  Each buffer is nominally 244 bytes, matching the BLE buffer size.  The third buffer points to the first buffer as it can easily be reused.  A further reduction in required memory of one buffer could be accomplished by encoding the payload directly into the outer message structure, but this makes the code rather confusing so this optimization is not shared.

1. **sCr_encoded_message_buffer**
   This holds the received encoded prompt message to be decoded.

2. **cr_ReachMessage sCr_uncoded_message_structure**
   This contains the encoded prompt separated from the header.  The same buffer is used for encoding.

3. **sCr_decoded_prompt_buffer**
   This contains the decoded prompt to be processed.  It reuses the memory allocated for the sCr_encoded_message_buffer.

4. **sCr_uncoded_response_buffer**
   Contains the raw payload data to be encoded.

5. **sCr_encoded_payload_buffer**
   Contains the encode payload data. The encoded payload is currently copied into the **sCr_uncoded_message_structure**.

6. **sCr_encoded_response_buffer**
   This contains the encoded message ready to be transmitted.
...
The cr_DeviceInfoResponse message includes the “sizes_struct” member.  This sizes structure is designed to let the server device tell the client about its allowed buffer sizes.  The default values describe the BLE implementation we are using, but a particular server could choose to use different sizes to save memory or to support a different transport protocol.
...
* Setting the minimum notification interval to zero can result in a flood of notifications that overwhelm the transport.  Server devices are advised to code their own minimum interval to be applied when the client sets it to zero.
