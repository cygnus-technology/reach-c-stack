...
3.4: February 2025: Improved coverage of the ahsoka header, extended parameters and error handling.
...
Parameters are described using a union of 11 distinct parameter types (cr_ParameterInfo). The union is designed to share the common parts of a parameter description while making room for specific details in a strongly typed fashion. The concept of "extended parameters" (cr_ParamExKey) are used to supply more data for those parameters that need it without bloating the basic parameters. While all of the description required for a float fits in the basic parameter description, an enumerated parameter requires names for each of the enumerations. The names of these enumerations are stored as a cr_ParamExKeystructure.
...
The i3_log module includes the i3_assert() macro. Like the basic macro, it halts if the expression is false. But care is taken to be sure that the halting error condition is easily identified. The implementation of the error handler is weak, so integrations are expected to ensure helpful behavior on an error. This includes halting at a debugger breakpoint, printing to the console and flashing LEDs.
